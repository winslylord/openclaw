...
export async function handleFeishuMessage(params: {
  ...
  try {
    ...
    let peerId: string = isGroup ? (groupSession?.peerId ?? ctx.chatId) : ctx.senderOpenId;
    ...
    if (sessionIsGroup && ctx.rootId) {
      const topicGroupConfig = resolveFeishuGroupConfig({ cfg: feishuCfg, groupId: ctx.chatId });
      const topicSessionMode =
        topicGroupConfig?.topicSessionMode ?? feishuCfg?.topicSessionMode ?? "disabled";
      ...
      try {
        ...
        if (quotedMessageInfo) {
          ...
          if (quotedMediaTypes.includes(quotedMessageInfo.contentType)) {
            log(
              `feishu[${account.accountId}]: quoted message is ${quotedMessageInfo.contentType}, downloading media`,
            );
            ...
            quotedContent = `[Quoted ${quotedMessageInfo.contentType}]`;
          } else {
    ...
    const historyKey = sessionIsGroup ? (groupHistoryKey ?? ctx.chatId) : undefined;
