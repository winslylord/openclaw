...
- When invoking via OpenClaw's exec tool, set a higher timeout to avoid premature termination (e.g., exec timeout=300).
- If no model is explicitly specified, the script picks the first available one from ComfyUI.

## Configuration

Set `COMFYUI_URL` env var or `skills.comfyui.env.COMFYUI_URL` in `~/.openclaw/openclaw.json` to override the server URL.
